...
TEST_CASE("Loading the first part of the wiki dataset works", "[parsing]") {
    Graph gIO = Graph("./tests/mock-data/Vertices.csv", "./tests/mock-data/Edges.txt");
    CHECK(gIO.num_vertices < 20);
}

TEST_CASE("Loading data automatically generated by the python script works", "[parsing]") {
...
TEST_CASE("BFS Shortest Path queried by strings returns correct path if path exists", "[parsing][bfs]") {
    Graph g;
...
TEST_CASE("The graph can find string keys generated from a vertex file", "[parsing]") {
